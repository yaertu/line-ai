...
const recordingDirectory = path.join(tmpdir(), `line-ai-v070-${Date.now()}`);
...
const videoTarget = path.join(outputDirectory, "line-ai-v070-yenilikler.mp4");
const posterTarget = path.join(outputDirectory, "line-ai-v070-yenilikler-poster.png");
const evidenceTarget = path.join(outputDirectory, "line-ai-v070-yenilikler.evidence.json");
...
if (!executablePath) {
	throw new Error("Google Chrome bulunamadı; v0.7.0 kaynak arayüz kaydı üretilemedi.");
}
...
try {
	...
	await page.getByRole("heading", { name: "Yenilikler · v0.7.0" }).waitFor();
	...
	if (!capturedVideo) {
		throw new Error("v0.7.0 kaynak arayüz kaydı başlatılamadı.");
	}

	const recordedPath = await capturedVideo.path();
	await access(recordedPath, constants.R_OK);
	const webmTarget = path.join(outputDirectory, "line-ai-v070-yenilikler.webm");
	...
	const evidence = {
		...
		source: {
			...
			url: applicationUrl,
			viewport: "1440x900",
			surface: "Ayarlar > Hakkında > Yenilikler · v0.7.0",
		},
		claims: {
			shown: [
				"tek Line AI Engine ve bulut API özeti",
				"sohbet, dosya, kod Artifact ve görsel çalışma alanı",
				"Ultra Premium grafit ve mint arayüz ile yeni Line AI simgesi",
				"canlı anahtar sağlığı, kalan kota ve maliyet denetimi",
			],
			notShownAsProductUi: [
