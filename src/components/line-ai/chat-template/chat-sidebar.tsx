...
export const ChatSidebar = ({
	...
	const groups = useMemo(() => {
		...
		return [
			...(pinned.length > 0
				? [["Sabitlenenler", pinned] as [string, ChatConversation[]]]
				: []),
			...byGroup.entries(),
		];
	}, [conversations, query]);
		...
		>
			...
			<label className="flex items-center gap-2 rounded-xl border border-border/60 bg-background/75 px-2.5 py-2 shadow-black/5 shadow-xs transition-colors focus-within:border-primary/45 focus-within:bg-background">
				...
				<input
					...
					value={query}
				/>
				<kbd className="hidden shrink-0 whitespace-nowrap rounded-md border border-border/70 bg-muted px-1.5 py-0.5 font-medium text-[0.6rem] text-muted-foreground min-[340px]:inline">
					Ctrl K
				</kbd>
			</label>
			...
			>
				<span
					aria-hidden="true"
					className="pointer-events-none absolute inset-y-0 -left-1/3 w-1/3 -skew-x-12 bg-gradient-to-r from-transparent via-white/15 to-transparent transition-transform duration-500 group-hover:translate-x-[430%]"
				/>
			...
			{contextMenu ? (
				...
				>
					...
					<ContextAction
						icon={
							conversations.find(
								(conversation) => conversation.id === contextMenu.id,
							)?.pinned ? (
