...
const loadPreferences = (): AppPreferences => {
	try {
		const parsed = JSON.parse(
			localStorage.getItem(PREFERENCES_STORE_KEY) ?? "null",
		...
		return {
			...
			browserTools:
				typeof parsed.browserTools === "boolean"
					? parsed.browserTools
					: DEFAULT_PREFERENCES.browserTools,
			chatFontSize: boundedNumber(
				parsed.chatFontSize,
				DEFAULT_PREFERENCES.chatFontSize,
				13,
				20,
			),
			codeFontSize: boundedNumber(
				parsed.codeFontSize,
			...
			responseStyle: ["balanced", "concise", "detailed"].includes(
				parsed.responseStyle ?? "",
			)
				? (parsed.responseStyle as AppPreferences["responseStyle"])
				: DEFAULT_PREFERENCES.responseStyle,
			uiFontSize: boundedNumber(
				parsed.uiFontSize,
...
const ChatTemplate = ({
	...
	const [activeId, setActiveId] = useState(
		() =>
			defaultConversationId ??
			conversationItems.find((conversation) => !conversation.archived)?.id ??
	...
	const [cloudMessage, setCloudMessage] = useState(
		"Line AI Cloud bağlantısı hazırlanıyor.",
	);
	...
	useEffect(() => {
		...
		const applyTheme = () => {
			const dark =
				...
				(preferences.theme === "system" && media?.matches === true);
			document.documentElement.classList.toggle("dark", dark);
			document.documentElement.style.colorScheme = dark ? "dark" : "light";
		};
		applyTheme();
		document.documentElement.style.setProperty(
			"--line-ai-ui-font-size",
			`${preferences.uiFontSize}px`,
		);
		document.documentElement.style.setProperty(
			"--line-ai-chat-font-size",
			`${preferences.chatFontSize}px`,
		);
		document.documentElement.style.setProperty(
			"--line-ai-composer-font-size",
			`${Math.max(14, preferences.chatFontSize)}px`,
		);
		document.documentElement.style.setProperty(
			"--line-ai-code-font-size",
	...
	const restoreArchivedConversation = (id: string) => {
		setConversationItems((current) =>
			current.map((conversation) =>
				conversation.id === id
					? { ...conversation, archived: false, updatedAt: nowIso() }
					: conversation,
			),
		);
	};
	...
	const deleteConversation = (id: string) => {
		const deletedIndex = conversationItems.findIndex(
	...
	const voteTurn = (turnId: string, vote: "up" | "down") => {
		setConversationItems((current) =>
			current.map((conversation) =>
				conversation.id === activeId
					? {
							...
							turns: conversation.turns.map((turn) =>
								turn.id === turnId && turn.from === "assistant"
									? {
											...turn,
											feedback: turn.feedback === vote ? undefined : vote,
										}
									: turn,
							),
							updatedAt: nowIso(),
	...
	const exportData = () => {
		...
		link.download = `line-ai-${new Date().toISOString().slice(0, 10)}.json`;
		link.href = url;
		link.click();
	...
	const importData = async (file: File) => {
		if (file.size > 10 * 1024 * 1024)
			throw new Error("İçe aktarma dosyası en fazla 10 MiB olabilir.");
		...
		if (
			parsed.format !== "line-ai-export-v1" ||
			!Array.isArray(parsed.conversations)
		) {
			throw new Error(
				"Bu dosya geçerli bir Line AI dışa aktarma paketi değil.",
			);
		}
		const imported = parsed.conversations
			.filter((value): value is ChatConversation => {
				...
				return (
					...
					candidate.turns.every(isStoredTurn)
				);
			})
			.slice(0, 100);
		if (imported.length !== parsed.conversations.length)
			throw new Error("Paket doğrulanamadı; geçersiz sohbet verisi bulundu.");
		setConversationItems((current) =>
			mergeConversationHistories(imported, current),
		);
	...
	const restoreDeletedConversation = () => {
		...
		setConversationItems((current) => {
			if (
				current.some(
					(conversation) =>
						conversation.id === deletedConversation.conversation.id,
				)
			)
				return current;
			const restored = [...current];
			restored.splice(
				Math.min(deletedConversation.index, restored.length),
				0,
	...
	return (
		...
		>
			...
			{isSettingsOpen ? (
				<SettingsPanel
					archivedConversations={conversationItems.filter(
						(conversation) => conversation.archived,
					)}
					cloudMessage={cloudMessage}
					cloudState={cloudState}
					conversationCount={conversationItems.length}
					messageCount={conversationItems.reduce(
				...
				{deletedConversation ? (
					<motion.div
						...
						transition={
							shouldReduceMotion
								? { duration: 0 }
								: { bounce: 0, duration: 0.22, type: "spring" }
						}
					>
						<div className="flex items-center gap-3 px-4 py-3">
							<span className="min-w-0 flex-1">
								...
								<span className="block truncate text-muted-foreground text-xs">
									{deletedConversation.conversation.title}
								</span>
							</span>
							<button
								className="inline-flex items-center gap-1.5 rounded-lg bg-primary/12 px-2.5 py-1.5 font-medium text-primary text-xs hover:bg-primary/20"
								onClick={restoreDeletedConversation}
								type="button"
							>
