...
try {
	...
	assert(
		landingHtml.includes("/media/line-ai-v070-yenilikler.mp4") &&
			landingHtml.includes("/media/line-ai-v070-yenilikler-poster.png") &&
			landingHtml.includes("releases/download/v0.7.0/Line.AI.exe"),
		"Landing v0.7.0 kaynak arayüz kaydını veya sabit indirme bağlantısını kullanmıyor.",
	);
	...
	const sourceUiEvidenceResponse = await requestRaw(
		"/media/line-ai-v070-yenilikler.evidence.json",
	);
	const sourceUiEvidence = await sourceUiEvidenceResponse.json();
	assert(sourceUiEvidenceResponse.status === 200, "v0.7.0 kaynak arayüz evidence JSON yayınlanmıyor.");
	assert(
		sourceUiEvidence?.source?.kind === "vite-source-ui-playwright" &&
			sourceUiEvidence?.source?.surface === "Ayarlar > Hakkında > Yenilikler · v0.7.0" &&
			sourceUiEvidence?.claims?.notShownAsProductUi?.includes("yönetici giriş bilgileri veya Engine anahtarı"),
		"v0.7.0 kaynak arayüz kaydının kapsamı ve sınırı doğrulanamıyor.",
	);

	const sourceUiVideoResponse = await requestRaw("/media/line-ai-v070-yenilikler.mp4");
	...
	assert(sourceUiVideoResponse.status === 200, "v0.7.0 kaynak arayüz videosu yayınlanmıyor.");
	assert(
		sourceUiVideoHash === sourceUiEvidence?.video?.sha256,
		"v0.7.0 kaynak arayüz videosu ile evidence SHA-256 özeti eşleşmiyor.",
	);
